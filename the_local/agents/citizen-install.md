...
12. Check the members page against these facts, and tell the developer about any that the host does not meet.
   ...
   - Taking a role that is the only way anyone in the current account holds the members capability gets a 403 response.
   ...
   - Removing a member outside the viewer's reach, one the host says cannot be removed, or the only member in the current account who holds the members capability gets a 403 response.
...
13. Check the role pages against these facts, and tell the developer about any that the host does not meet.
   ...
   - The role pages do not use the viewer's rank. A member who holds the roles capability can edit every role in the current account and give any role any rank, including a role they hold.
   - Saving a role with the members capability unticked gets a 403 response when that role grants the members capability and no member in the current account holds the members capability through any other role. This holds even when no member holds the role being saved. The form still shows the checkbox, so the refusal is seen only on saving.
   - Tell the developer that a member holding both the roles capability and the members capability can raise a role they hold to the highest rank and so reach every member on the members page.
