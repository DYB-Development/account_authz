...
description: Use to hook citizen into a project — adding the gem, installing its migrations, choosing the account membership model and including Citizen::Member in it, including Citizen::Authorization in controllers, setting Citizen::Current.account_id per request, and mounting Citizen::Engine for the members and role pages.
...
- `Citizen::Member` — a model concern included in the host model that holds roles, normally the record for a person inside one account, giving it role assignments.
...
6. Choose the model that holds roles. A role belongs to one account, so the model to include `Citizen::Member` in is the record for a person inside one account, such as `Membership`, not the person record, such as `User`.
   ...
   - Add `include Citizen::Member` to the chosen model's file in `app/models/`. Any Active Record model works, because the assignment is polymorphic. Destroying a member record destroys its role assignments, so with a membership model, a person's roles in an account go when their membership in it goes.
...
12. Check the members page against these facts, and tell the developer about any that the host does not meet.
    ...
    - Every member record the page lists must respond to `name` and `email` and include `Citizen::Member`. If the model from step 6 lacks `name` or `email`, as a membership model often does, ask the developer how it should answer them, such as from the person record it belongs to, before continuing.
