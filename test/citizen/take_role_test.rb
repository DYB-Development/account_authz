...
module Citizen
  class TakeRoleTest < ActiveSupport::TestCase
    ...
    test "taking a role removes it from the person" do
      ...
      assert_not_includes @person.reload.citizen_roles.in_account(1), @role
    end

    test "taking the last role that manages members is refused" do
      result = TakeRole.new(person: @manager, account: 1, values: { member_id: @manager.id, role_id: @manager.citizen_roles.first.id }).call
