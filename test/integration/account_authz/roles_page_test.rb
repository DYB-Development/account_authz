...
module AccountAuthz
  class RolesPageTest < ActionDispatch::IntegrationTest
    ...
    test "a member who can manage roles opens the roles page" do
      get "/account_authz/roles", params: { signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "a member who cannot manage roles is refused the roles page" do
      ...
      get "/account_authz/roles", params: { signed_in_member_id: member.id, account_id: 1 }
    ...
    test "the roles page opens for the capability the app names for managing roles" do
      ...
      get "/account_authz/roles", params: { signed_in_member_id: lead.id, account_id: 1 }
    ...
    test "a member who can manage roles creates a role in the current account" do
      post "/account_authz/roles", params: { role: { name: "Pretend Role" }, signed_in_member_id: @admin.id, account_id: 1 }

      assert Role.in_account(1).exists?(name: "Pretend Role")
    end

    test "a new role holds the capabilities chosen for it" do
      post "/account_authz/roles", params: { role: { name: "Pretend Role", capabilities: %w[view_reports] }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "a member who can manage roles adds a role from a template" do
      AccountAuthz.templates { template :reporter, capabilities: %w[view_reports] }

      post "/account_authz/roles", params: { template: "reporter", signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "a member who cannot manage roles cannot create a role" do
      ...
      post "/account_authz/roles", params: { role: { name: "Pretend Role" }, signed_in_member_id: member.id, account_id: 1 }
    ...
    test "a member who can manage roles renames a role" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Renamed Role" }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "unticking every capability on a role leaves it with none" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Pretend Role", capabilities: [ "" ] }, signed_in_member_id: @admin.id, account_id: 1 }

      assert_empty role.reload.capabilities
    end

    test "the new role form offers a checkbox for each capability in the catalog" do
      get "/account_authz/roles/new", params: { signed_in_member_id: @admin.id, account_id: 1 }

      assert_equal %w[manage_roles view_reports], css_select("form[action='/account_authz/roles'] input[type=checkbox][name='role[capabilities][]']").map { |box| box["value"] }
    end

    test "the edit role form ticks the capabilities the role holds" do
      role = Role.create!(account_id: 1, name: "Pretend Role", capabilities: %w[view_reports])

      get "/account_authz/roles/#{role.id}/edit", params: { signed_in_member_id: @admin.id, account_id: 1 }

      assert_equal %w[view_reports], css_select("form[action='/account_authz/roles/#{role.id}'] input[type=checkbox][checked]").map { |box| box["value"] }
    end

    test "the roles page offers to add each default template" do
      AccountAuthz.templates { template :reporter, capabilities: %w[view_reports], default: true }

      get "/account_authz/roles", params: { signed_in_member_id: @admin.id, account_id: 1 }

      assert_select "form[action='/account_authz/roles'][method=post] input[name=template][value=reporter]"
    end
    ...
    test "the roles page links to each role's edit form" do
      ...
      assert_select "a[href=?]", "/account_authz/roles/#{role.id}/edit"
    end

    test "the roles page links to the new role form" do
      get "/account_authz/roles", params: { signed_in_member_id: @admin.id, account_id: 1 }

      assert_select "a[href=?]", "/account_authz/roles/new"
    end
    ...
    test "a member who can manage roles sets a role's rank" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Pretend Role", rank: "2" }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "the members capability cannot be taken off the only role that lets anyone manage members" do
      ...
      patch "/account_authz/roles/#{manager_role.id}", params: { role: { name: "Manager", capabilities: [ "" ] }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "an editor is refused a new role with a capability they do not hold" do
      AccountAuthz.catalog { permission :export_data }

      post "/account_authz/roles", params: { role: { name: "Pretend Role", capabilities: %w[export_data] }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "an editor is refused adding a capability they do not hold to a role" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Pretend Role", capabilities: %w[export_data] }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "an editor is refused setting a role's rank above their own" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Pretend Role", rank: "5" }, signed_in_member_id: @admin.id, account_id: 1 }

      assert_equal 0, role.reload.rank
    end

    test "an editor is refused a new role ranked above their own" do
      post "/account_authz/roles", params: { role: { name: "Pretend Role", rank: "5" }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "an editor is refused changing a role ranked above their own" do
      ...
      patch "/account_authz/roles/#{role.id}", params: { role: { name: "Renamed" }, signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "an editor is refused a template role with a capability they do not hold" do
      AccountAuthz.catalog { permission :export_data }
      AccountAuthz.templates { template :exporter, capabilities: %w[export_data] }

      post "/account_authz/roles", params: { template: "exporter", signed_in_member_id: @admin.id, account_id: 1 }
    ...
    test "the roles page links back to the members page" do
      ...
      assert_select "a[href='/account_authz/members']", text: "Members"
    end
  end
end
