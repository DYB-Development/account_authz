...
module AccountAuthz
  class InvitationsTest < ActionDispatch::IntegrationTest
    ...
    test "a manager invites a person to the account by name and email" do
      post "/account_authz/invitations", params: { invitation: { name: "Pretend Person", email: "pretend@example.com" }, signed_in_member_id: @manager.id, account_id: 1 }
    ...
    test "a member who cannot manage members cannot invite anyone" do
      ...
      post "/account_authz/invitations", params: { invitation: { name: "Pretend Guest", email: "guest@example.com" }, signed_in_member_id: person.id, account_id: 1 }
    ...
    test "a manager cancels an invitation still waiting for an answer" do
      ...
      delete "/account_authz/invitations/#{invitation.id}", params: { signed_in_member_id: @manager.id, account_id: 1 }
    ...
    test "a manager sends an invitation again" do
      ...
      post "/account_authz/invitations/#{invitation.id}/resend", params: { signed_in_member_id: @manager.id, account_id: 1 }
