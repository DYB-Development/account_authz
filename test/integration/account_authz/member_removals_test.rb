...
module AccountAuthz
  class MemberRemovalsTest < ActionDispatch::IntegrationTest
    ...
    test "a manager removes a member from the account" do
      delete "/account_authz/members/#{@person.id}", params: { signed_in_member_id: @manager.id, account_id: 1 }
    ...
    test "a removed member loses the roles they held in the account" do
      ...
      assert_empty @person.account_authz_roles.in_account(1)
    end
    ...
    test "a member who cannot manage members cannot remove anyone" do
      ...
      delete "/account_authz/members/#{@manager.id}", params: { signed_in_member_id: @person.id, account_id: 1 }

      assert_equal 1, @manager.reload.account_id
    end

    test "the only manager cannot be removed from the account" do
      delete "/account_authz/members/#{@manager.id}", params: { signed_in_member_id: @manager.id, account_id: 1 }
